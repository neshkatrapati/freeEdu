        ...
        include_once("../../lib/connection.php");
            ...
            while($subj=mysql_fetch_array($lib))
            {
                ...
                if($search==NULL)
                   ...
                   // echo  "select * from MLIBRARYT where bname like '%$search%' or bookid like '%$search%' or bauthor like '%$search%' or bpub like '%$search%' or bedition like '%$search%'";
                   //echo mysql_num_rows($library);
                    while($l=mysql_fetch_array($library))
                    {
                        ...
                        echo "</td>";
                        $oidobj=getCurrentObject();
                        $cklid=mysql_query("select * from MORDERT where oid='$oidobj' and lid='$lid' and aop='-1'");
                        $cklid1=mysql_query("select * from MORDERT where oid='$oidobj' and lid='$lid' and aop='a'");
                        $ck_num=mysql_num_rows($cklid);
                        $ck_num1=mysql_num_rows($cklid1);
                        if($ck_num1==0)
                        {
                            if($ck_num==0)
